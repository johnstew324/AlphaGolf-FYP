...
def memoize_with_timeout(timeout=3600):  # 1 hour timeout by default
    """Memoization decorator with timeout to prevent memory leaks"""
    cache = {}
    timestamps = {}
...
# Apply memoization to data extraction functions
...
def create_player_registry(data_extractor, tournament_id, history_tournament_id, season, player_id_list=None, batch_size=None):
    """
    Create a player registry for a tournament to ensure consistent player entries.
    
    Args:
        data_extractor: DataExtractor instance
        tournament_id: Standard tournament ID
        history_tournament_id: Special ID for tournament history
        season: Tournament season
        player_id_list: Optional list of player IDs to filter by
        batch_size: Optional maximum number of players to process
        
    Returns:
        DataFrame with one row per player containing player metadata
    """
...
def safely_merge_features(base_df, feature_df, on='player_id', processor_name=None):
    """
    Safely merge feature dataframes, handling duplicates and conflicts.
    
    Args:
        base_df: Base dataframe to merge into
        feature_df: Feature dataframe to merge from
        on: Column(s) to merge on
        processor_name: Optional name of processor for suffixes
...
def process_single_tournament_improved(tournament_id, player_id_list=None, player_batch_size=None):
    """
    Process a single tournament with improved handling of player data.
    
    This version creates a consistent player registry first, then processes
    features for each player only once to avoid duplication.
    
    Args:
        tournament_id: Tournament ID to process
        player_id_list: Optional list of player IDs to filter by
        player_batch_size: Maximum number of players to process
        
    Returns:
        Tuple of (tournament_id, combined_features)
    """
    ...
    # Extract the year from the tournament ID (assuming format RYYYYTTT)
    if tournament_id.startswith("R") and len(tournament_id) >= 8:
        season = int(tournament_id[1:5])
    else:
        # Default season if can't extract from ID
        season = 2025
        
    print(f"Extracted season: {season}")
    
    # For tournament history, we need to handle the special format
    # Tournament history uses R2025TTT format regardless of actual year
    history_tournament_id = tournament_id
    if tournament_id.startswith("R") and len(tournament_id) >= 8:
        # Extract the tournament part (TTT) from the original ID
        tournament_part = tournament_id[5:]
        # Create the special format for tournament history (always using 2025)
...
def test_feature_generation_improved(
    ...
    """
    Improved version of feature generation with better handling of player data.
    
    Args:
        tournament_ids: List of tournament IDs to test
        player_id_list: Optional list of specific player IDs to include
        tournament_batch_size: Number of tournaments to process in parallel
        player_batch_size: Maximum number of players to include per tournament
        max_workers: Maximum number of parallel processes
        resume: Whether to resume from previous runs
        
    Returns:
        Combined dataframe with all tournament features
    """
    ...
    # Skip completed tournaments if resuming
    if resume:
    ...
    # Store all combined features (one per tournament)
    all_tournament_dfs = []
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    # Update progress
                    tracker.update_tournament_progress(tournament_id)
                except Exception as e:
...
if __name__ == "__main__":
    ...
    # Load tournament IDs from file if it exists (for large lists)
    ...
    # Define a list of player IDs that you're interested in
    player_ids = ['49303', '33948', '52370', '52955', '22371', '40026', '45522', '56630', '40058', '33141', '34255', '28420', '60004', '48084', '55708', '29268', '47504', '46443', '32070', 
                  '35450', '49590', '59440', '34021', '52372', '58933', '32366', '20229', '51766', 
                '47591', '49453', '39997', '59442', '59836', '46435', '32982', '52514', '58605', '39067', '34076', '45157', '28089', '39859', '33653', '51690', '50497', '45609', '59866', 
    ...
    # Load player IDs from file if it exists (for large lists)
    ...
    combined_df = test_feature_generation_improved(
        ...
        tournament_batch_size=5,    # Process 5 tournaments at a time
        player_batch_size=50,       # Limit to 50 players per tournament if needed
